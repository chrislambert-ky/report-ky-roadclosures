...
// This was very tricky for me. I simply did not have time or skills to troubleshoot the problem.
...
async function main()
{
  ...
  console.log('Fetching CSV from:', csvUrl); // display for the terminal user
  const csvText = await fetchCSV(csvUrl);
  console.log('CSV fetched. Length:', csvText.length);// display for the terminal user
  
  const filteredRows = filterRows(csvText);
  console.log('Rows parsed:', filteredRows.length);// display for the terminal user
  if (filteredRows.length > 0) {
    console.log('First row:', filteredRows[0]);// display for the terminal user
  }
  ...
  if (fs.existsSync(outputFile)) {
    console.log('Output file will be overwritten at the end of processing.'); // display for the terminal user
  }
...
async function processBatch(batch, batchStartIdx)
{
  ...
  while (idx < filteredRows.length) {
    ...
    console.log(`Batch ${Math.floor(idx / BATCH_SIZE) + 1}: ${enrichedBatch.length} records processed. Total so far: ${allEnriched.length}`); // display for the terminal user
    idx += BATCH_SIZE;
  }
  ...
  console.log(`All records processed. Total: ${allEnriched.length} records written to ${outputFile}`); // display for the terminal user
  console.log(`Processing time: ${elapsedSeconds} seconds`); // display for the terminal user
  
  if (allEnriched.length > 0) {
    console.log('Sample enriched row:', allEnriched[0]); // display for the terminal user
  }
}
