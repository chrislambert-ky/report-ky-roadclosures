...
// DESIGN DECISION: I used async/await for network requests because that seemed like it would work.
...
{
  // Converts the CSV text into an array of objects (one for each row)
  ...
  return records.map(row => (
    ...
    Reported_On: row.Reported_On, // When the closure was first reported
    End_Date: row.End_Date, // When the closure ended (if known)
    Duration_Hours: row.Duration_Hours // Duration (hours) - this is calculated in the previous python script.
    }));
}
...
// The API docs provide Python code to better understand the API and hopefully recreate the same functionality
...
{
  // The API doesn't work without latitude and longitude (sort of - it's complicated).
